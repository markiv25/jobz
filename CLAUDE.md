...
- `{"status": "update-available", "local": "1.0.0", "remote": "1.1.0", "changelog": "..."}` → tell the user:
  > "jobz update available (v{local} → v{remote}). Your data (CV, profile, tracker, reports) will NOT be touched. Want me to update?"
  If yes → run `node update-system.mjs apply`. If no → run `node update-system.mjs dismiss`.
- `{"status": "up-to-date"}` → say nothing
- `{"status": "dismissed"}` → say nothing
- `{"status": "offline"}` → say nothing

The user can also say "check for updates" or "update jobz" at any time to force a check.
...
|---------|------------------------|-------------|
| `/jobz` | `/jobz` | Show menu or evaluate JD with args |
| `/jobz-pipeline` | `/jobz pipeline` | Process pending URLs from inbox |
| `/jobz-evaluate` | `/jobz oferta` | Evaluate job offer (A-F scoring) |
| `/jobz-compare` | `/jobz ofertas` | Compare and rank multiple offers |
| `/jobz-contact` | `/jobz contacto` | LinkedIn outreach (find contacts + draft) |
| `/jobz-deep` | `/jobz deep` | Deep company research |
| `/jobz-pdf` | `/jobz pdf` | Generate ATS-optimized CV |
| `/jobz-training` | `/jobz training` | Evaluate course/cert against goals |
| `/jobz-project` | `/jobz project` | Evaluate portfolio project idea |
| `/jobz-tracker` | `/jobz tracker` | Application status overview |
| `/jobz-apply` | `/jobz apply` | Live application assistant |
| `/jobz-scan` | `/jobz scan` | Scan portals for new offers |
| `/jobz-batch` | `/jobz batch` | Batch processing with parallel workers |
| `/jobz-patterns` | `/jobz patterns` | Analyze rejection patterns and improve targeting |
| `/jobz-followup` | `/jobz followup` | Follow-up cadence tracker |

**Note:** OpenCode commands invoke the same `.claude/skills/jobz/SKILL.md` skill used by Claude Code. The `modes/*` files are shared between both platforms.
...
> - Run `/jobz scan` (or `/jobz-scan` if using OpenCode) to search portals
> - Run `/jobz` to see all commands
>
...
If the user accepts, use the `/loop` or `/schedule` skill (if available) to set up a recurring `/jobz scan` (or `/jobz-scan` if using OpenCode). If those aren't available, suggest adding a cron job or remind them to run `/jobz scan` (or `/jobz-scan` if using OpenCode) periodically.
