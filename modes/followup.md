...
> "No active applications to follow up on. Apply to some roles first with `/jobz` and come back when they're aging."
...
- Suggest the user run `/jobz contacto {company}` to find the right person first
...
> - Trying a different contact via `/jobz contacto`
...
**To:** {email or "No contact found — run `/jobz contacto` first"}
